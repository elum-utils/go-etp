...
func TestSessionCapabilityEnforcementRejectsUnsupportedRequest(t *testing.T) {
	...
	hello := NewFrame(FrameHelloAck, SchemaHello, EncodeHelloMessage(Hello{Role: RoleServer, Capabilities: CapabilityAck}))
...
func TestSessionTransferResumeDefaultRejectsWithTransferState(t *testing.T) {
	transport := newRecordingTransport(t)
	session := NewSession(transport)
	...
	if state.TransferID != 99 || state.Flags != TransferStateFlagResumeRejected {
