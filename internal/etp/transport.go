package etp

import (
	"encoding/binary"
	"errors"
	"fmt"
	"io"
	"net"
	"sync"
	"sync/atomic"
	"time"
)

const MaxFrameBytes = 8 << 20

var ErrSlowloris = errors.New("slowloris detected")

type SlowlorisConfig struct {
	LengthTimeout    time.Duration
	FrameGrace       time.Duration
	MinReadRate      int64
	MaxFrameBytes    uint32
	DisableDeadlines bool
}

func DefaultSlowlorisConfig() SlowlorisConfig {
	return SlowlorisConfig{
		LengthTimeout: 2 * time.Second,
		FrameGrace:    2 * time.Second,
		MinReadRate:   8 * 1024,
		MaxFrameBytes: MaxFrameBytes,
	}
}

type FrameTransport interface {
	SendFrame(frame []byte) error
	ReadFrame() ([]byte, error)
	Close() error
}

type LeasedFrameTransport interface {
	ReadFrameLease() (*FrameLease, error)
}

// NegotiatedFrameTransport is for carriers that establish ETP capabilities
// out of band. Regular transports must negotiate them with Hello/HelloAck.
type NegotiatedFrameTransport interface {
	NegotiatedCapabilities() uint64
}

type FrameLimitTransport interface {
	SetMaxFrameBytes(uint32)
}

type DeadlineStream interface {
	io.Reader
	io.Writer
	io.Closer
	SetReadDeadline(time.Time) error
	SetWriteDeadline(time.Time) error
}

type deadlineStream = DeadlineStream

type StreamTransport struct {
	conn      deadlineStream
	mu        sync.Mutex
	writeLen  [4]byte
	readMu    sync.Mutex
	readLen   [4]byte
	guard     SlowlorisConfig
	maxFrame  atomic.Uint32
	deadlines bool
}

func NewStreamTransport(conn net.Conn) *StreamTransport {
	return NewStreamTransportWithSlowlorisGuard(conn, DefaultSlowlorisConfig())
}

func NewStreamTransportWithSlowlorisGuard(conn net.Conn, guard SlowlorisConfig) *StreamTransport {
	return NewStreamTransportForStream(conn, guard)
}

func NewStreamTransportForStream(conn deadlineStream, guard SlowlorisConfig) *StreamTransport {
	if guard.LengthTimeout <= 0 {
		guard.LengthTimeout = 2 * time.Second
	}
	if guard.FrameGrace <= 0 {
		guard.FrameGrace = 2 * time.Second
	}
	if guard.MinReadRate <= 0 {
		guard.MinReadRate = 8 * 1024
	}
	if guard.MaxFrameBytes == 0 || guard.MaxFrameBytes > MaxFrameBytes {
		guard.MaxFrameBytes = MaxFrameBytes
	}
	t := &StreamTransport{
		conn:      conn,
		guard:     guard,
		deadlines: !guard.DisableDeadlines,
	}
	t.maxFrame.Store(guard.MaxFrameBytes)
	return t
}

func (t *StreamTransport) SendFrame(frame []byte) error {
	if len(frame) > MaxFrameBytes {
		return errors.New("frame exceeds max size")
	}
	t.mu.Lock()
	defer t.mu.Unlock()
	binary.BigEndian.PutUint32(t.writeLen[:], uint32(len(frame)))
	if err := writeFull(t.conn, t.writeLen[:]); err != nil {
		return err
	}
	return writeFull(t.conn, frame)
}

func (t *StreamTransport) SetMaxFrameBytes(max uint32) {
	if max == 0 || max > MaxFrameBytes {
		max = MaxFrameBytes
	}
	t.maxFrame.Store(max)
}

func (t *StreamTransport) SetWriteDeadline(deadline time.Time) error {
	if !t.deadlines {
		return nil
	}
	return t.conn.SetWriteDeadline(deadline)
}

func (t *StreamTransport) ReadFrame() ([]byte, error) {
	return t.ReadFrameInto(nil)
}

func (t *StreamTransport) ReadFrameLease() (*FrameLease, error) {
	bufp, err := t.readFrameBuffer()
	if err != nil {
		return nil, err
	}
	return newPooledFrameLease(bufp), nil
}

func (t *StreamTransport) ReadFrameInto(dst []byte) ([]byte, error) {
	t.readMu.Lock()
	defer t.readMu.Unlock()
	if err := t.readFrameLength(); err != nil {
		return nil, t.classifyReadError("read frame length", err)
	}

	n := binary.BigEndian.Uint32(t.readLen[:])
	maxFrame := t.maxFrame.Load()
	if n < HeaderSize {
		return nil, fmt.Errorf("%w: incoming frame is smaller than header: %d", ErrInvalidFrame, n)
	}
	if n > maxFrame {
		return nil, fmt.Errorf("%w: incoming frame exceeds max size: %d > %d", ErrInvalidFrame, n, maxFrame)
	}

	if err := t.setReadDeadline(t.frameReadBudget(n)); err != nil {
		return nil, err
	}
	if cap(dst) < int(n) {
		dst = make([]byte, n)
	} else {
		dst = dst[:n]
	}
	frame := dst
	if _, err := io.ReadFull(t.conn, frame); err != nil {
		return nil, t.classifyReadError("read frame body", err)
	}
	_ = t.clearReadDeadline()
	return frame, nil
}

func (t *StreamTransport) readFrameBuffer() (*[]byte, error) {
	t.readMu.Lock()
	defer t.readMu.Unlock()
	if err := t.readFrameLength(); err != nil {
		return nil, t.classifyReadError("read frame length", err)
	}
	n := binary.BigEndian.Uint32(t.readLen[:])
	maxFrame := t.maxFrame.Load()
	if n < HeaderSize {
		return nil, fmt.Errorf("%w: incoming frame is smaller than header: %d", ErrInvalidFrame, n)
	}
	if n > maxFrame {
		return nil, fmt.Errorf("%w: incoming frame exceeds max size: %d > %d", ErrInvalidFrame, n, maxFrame)
	}
	if err := t.setReadDeadline(t.frameReadBudget(n)); err != nil {
		return nil, err
	}
	bufp := getFrameBuffer(int(n))
	*bufp = (*bufp)[:n]
	frame := *bufp
	if _, err := io.ReadFull(t.conn, frame); err != nil {
		putFrameBuffer(bufp)
		return nil, t.classifyReadError("read frame body", err)
	}
	_ = t.clearReadDeadline()
	return bufp, nil
}

func (t *StreamTransport) readFrameLength() error {
	if err := t.clearReadDeadline(); err != nil {
		return err
	}
	if _, err := io.ReadFull(t.conn, t.readLen[:1]); err != nil {
		return err
	}
	if err := t.setReadDeadline(t.guard.LengthTimeout); err != nil {
		return err
	}
	_, err := io.ReadFull(t.conn, t.readLen[1:])
	return err
}

func (t *StreamTransport) Close() error {
	return t.conn.Close()
}

func (t *StreamTransport) frameReadBudget(n uint32) time.Duration {
	bodyBudget := time.Duration(int64(n) * int64(time.Second) / t.guard.MinReadRate)
	return t.guard.FrameGrace + bodyBudget
}

func (t *StreamTransport) setReadDeadline(after time.Duration) error {
	if !t.deadlines {
		return nil
	}
	return t.conn.SetReadDeadline(time.Now().Add(after))
}

func (t *StreamTransport) clearReadDeadline() error {
	if !t.deadlines {
		return nil
	}
	return t.conn.SetReadDeadline(time.Time{})
}

func (t *StreamTransport) classifyReadError(op string, err error) error {
	var netErr net.Error
	if errors.As(err, &netErr) && netErr.Timeout() {
		return fmt.Errorf("%w: %s timeout", ErrSlowloris, op)
	}
	return err
}

func Send(t FrameTransport, frame Frame) error {
	if len(frame.Payload) > MaxFrameBytes-HeaderSize {
		return errors.New("frame exceeds max size")
	}
	size := EncodedFrameSize(frame)
	bufp := getFrameBuffer(size)
	data, err := EncodeFrameInto((*bufp)[:0], frame)
	if err != nil {
		putFrameBuffer(bufp)
		return err
	}
	err = t.SendFrame(data)
	putFrameBuffer(bufp)
	return err
}

func Read(t FrameTransport) (Frame, error) {
	if lt, ok := t.(LeasedFrameTransport); ok {
		lease, err := lt.ReadFrameLease()
		if err != nil {
			return Frame{}, err
		}
		frame, err := decodeFrameView(lease.Data, lease)
		if err != nil {
			lease.Release()
			return Frame{}, fmt.Errorf("%w: %v", ErrInvalidFrame, err)
		}
		return frame, nil
	}
	data, err := t.ReadFrame()
	if err != nil {
		return Frame{}, err
	}
	frame, err := DecodeFrameView(data)
	if err != nil {
		return Frame{}, fmt.Errorf("%w: %v", ErrInvalidFrame, err)
	}
	return frame, nil
}

func writeFull(w io.Writer, data []byte) error {
	for len(data) > 0 {
		n, err := w.Write(data)
		if n > 0 {
			data = data[n:]
		}
		if err != nil {
			return err
		}
		if n == 0 {
			return io.ErrShortWrite
		}
	}
	return nil
}
