...
const (
	...
	WireVersion         = uint8(1)
	HeaderSize          = 40
	MaxPooledFrameBytes = 256 << 10
)

const knownFrameFlags = FlagFirst | FlagLast | FlagAckRequest | FlagEncrypted | FlagCompressed | FlagControl

var ErrInvalidFrame = errors.New("invalid etp frame")
...
type FrameLease struct {
	...
	refs     atomic.Int32
	release  func([]byte)
	releaser FrameLeaseReleaser
	bufp     *[]byte
}

type FrameLeaseReleaser interface {
	ReleaseFrameLease([]byte)
}
...
func InitFrameLease(lease *FrameLease, data []byte, releaser FrameLeaseReleaser) *FrameLease {
	if lease == nil {
		panic("etp: nil frame lease")
	}
	if lease.refs.Load() != 0 {
		panic("etp: initializing an active frame lease")
	}
	lease.Data = data
	lease.release = nil
	lease.releaser = releaser
	lease.bufp = nil
	lease.refs.Store(1)
	return lease
}

func newPooledFrameLease(bufp *[]byte) *FrameLease {
	l := &FrameLease{Data: *bufp, bufp: bufp}
	l.refs.Store(1)
	return l
}

func (l *FrameLease) Retain() {
	if l == nil {
		return
	}
	for {
		refs := l.refs.Load()
		if refs <= 0 {
			panic("etp: retaining a released frame lease")
		}
...
func (l *FrameLease) Release() {
	...
	for {
		refs := l.refs.Load()
		if refs <= 0 {
			panic("etp: frame lease released more than retained")
		}
		if !l.refs.CompareAndSwap(refs, refs-1) {
			continue
		}
	...
	if l.releaser != nil {
		...
		l.Data = nil
		l.releaser = nil
		releaser.ReleaseFrameLease(data)
		return
	}
	if l.release != nil {
		l.release(l.Data)
...
func EncodeFrameInto(dst []byte, frame Frame) ([]byte, error) {
	...
	if err := validateHeaderValues(frame.Header); err != nil {
		return nil, err
	}
	if cap(dst) < size {
		dst = make([]byte, size)
...
func putFrameBuffer(bufp *[]byte) {
	if bufp == nil || cap(*bufp) > MaxPooledFrameBytes {
		return
	}
	*bufp = (*bufp)[:0]
...
func DecodeHeader(data []byte) (Header, error) {
	...
	if err := validateHeaderValues(h); err != nil {
		return Header{}, err
	}
	return h, nil
}

func validateHeaderValues(h Header) error {
	if h.FrameType < FrameData || h.FrameType > FrameTransferResume {
		return fmt.Errorf("unknown frame type: %d", h.FrameType)
	}
	if h.Priority > PriorityIdle {
		return fmt.Errorf("invalid frame priority: %d", h.Priority)
	}
	if h.ChannelID > ChannelBackground {
		return fmt.Errorf("invalid frame channel: %d", h.ChannelID)
	}
	if h.HeaderFlags != 0 {
		return fmt.Errorf("unsupported header flags: %#x", h.HeaderFlags)
	}
	if h.Flags&^uint16(knownFrameFlags) != 0 {
		return fmt.Errorf("unsupported frame flags: %#x", h.Flags)
	}
	if h.Flags&(FlagEncrypted|FlagCompressed) != 0 {
		return errors.New("encrypted and compressed frame flags are not implemented")
	}
	return nil
}

func DecodeFrameView(data []byte) (Frame, error) {
	return decodeFrameView(data, nil)
}

func decodeFrameView(data []byte, lease *FrameLease) (Frame, error) {
	if len(data) > MaxFrameBytes {
		return Frame{}, errors.New("frame exceeds max size")
	}
	h, err := DecodeHeader(data)
	if err != nil {
		return Frame{}, err
	}
	payloadOffset := int(h.PayloadOffset)
	if uint64(h.PayloadLength) > uint64(len(data)-payloadOffset) {
		return Frame{}, errors.New("payload exceeds frame length")
	}
	end := payloadOffset + int(h.PayloadLength)
	if end != len(data) {
		return Frame{}, errors.New("frame has trailing bytes")
	}
	return Frame{Header: h, Payload: data[h.PayloadOffset:end], lease: lease}, nil
}
