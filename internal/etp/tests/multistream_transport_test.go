...
func TestMultiStreamTransportOpensOneStreamForConcurrentColdChannel(t *testing.T) {
	opener := &memoryStreamOpener{blockReads: true}
	transport := NewMultiStreamTransport(MultiStreamTransportConfig{
		Context: context.Background(),
		Guard:   DefaultSlowlorisConfig(),
		OpenStream: func(context.Context) (DeadlineStream, error) {
	...
	encoded := mustEncodeFrame(t, frame)
	...
	for i := 0; i < senders; i++ {
		go func() {
			<-start
			errs <- transport.SendFrameOnChannel(ChannelControl, encoded)
		}()
	}
	close(start)
	for i := 0; i < senders; i++ {
	...
	if got := len(opener.streams()); got != 1 {
...
func (o *memoryStreamOpener) open() *memoryDeadlineStream {
	...
	if o.blockReads {
		stream.readBlock = make(chan struct{})
	}
	o.mu.Lock()
...
func (s *memoryDeadlineStream) Close() error {
	s.closeOnce.Do(func() {
		if s.readBlock != nil {
			close(s.readBlock)
		}
